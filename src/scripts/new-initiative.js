...
function handle(e) {
  if (e.target.tagName == "BUTTON" && [...e.target.classList].includes("remove")) {
    e.preventDefault();
  ...
  if (e.target.tagName == "BUTTON" && [...e.target.classList].includes("add")) {
    ...
    $(".leads-input")[0].insertBefore(button, [...$(".leads-input")[0].children].at(-1))
    $(".leads-input")[0].insertBefore(select, [...$(".leads-input")[0].children].at(-1))
  }
}
...
fetch(backend + "./users", {
  ...
  data = data.map(el => `<option value="${el.user_id}">${el.name} (${el.email})</option>`)
